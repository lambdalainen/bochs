...
{
  printf("### cursorWarped (shouldn't be called)\n");
  POINT pt = { 0, 0 };
...
#if DEBUG_MOUSE
  printf("### (x, y) (%i, %i), (sx, sy) (%i %i) (dx, dy) (%i %i)\n",
         x, y, ms_savedx, ms_savedy, ms_xdelta, ms_ydelta);
#endif

  if (bx_state!=mouse_button_state) {
    EnterCriticalSection(&stInfo.keyCS);
    printf("### bx_state %i, mouse_button_state %i\n", bx_state, mouse_button_state);
    printf("### calling enq_mouse_event 1\n"); // this only happens when mouse button is pressed
    enq_mouse_event();
...
void enq_mouse_event(void)
{
  ...
  if (ms_xdelta || ms_ydelta || ms_zdelta) {
    ...
    printf("==> enq: (%i, %i) * 2\n", ms_xdelta, ms_ydelta);
#endif
      // Yifan: *2 is a temporary solution
      current.mouse_x = ms_xdelta * 2;
      current.mouse_y = ms_ydelta * 2;
    }
...
void bx_win32_gui_c::handle_events(void)
{
  ...
  // Handle mouse moves
  //
  // This gets called repeatedly, but the function has:
  //
  //     if (ms_xdelta || ms_ydelta || ms_zdelta) { ... }
  enq_mouse_event();
  ...
  while (head != tail) {
    ...
    {
#if DEBUG_MOUSE
    printf("### DEV_mouse_motion (x, y, z): (%i, %i, %i), mouse_button_state: %i win32MouseModeAbsXY: %i\n",
                       queue_event->mouse_x, queue_event->mouse_y,
